...
public class CompostRegistry implements ICompostRegistry {
    ...
    public void registerCompostMaterial(ItemStack itemStack, ICompostMaterial materialInfo) {
        ...
        if (id != null && materialInfo != null) itemRegistry.register(id, materialInfo);
    }

    @Override
    public void registerCompostMaterial(String oreDictionaryKey, ICompostMaterial materialInfo) {
        if (oreDictionaryKey != null && materialInfo != null) oreDictRegistry.put(oreDictionaryKey, materialInfo);
    }

    @Override
    public void registerCompostMaterial(Class clazz, ICompostMaterial materialInfo) {
        if (clazz != null && materialInfo != null) classRegistry.put(clazz, materialInfo);
    }
    ...
    public void removeCompostMaterial(String oreDictionaryKey) {
        if (oreDictionaryKey != null) oreDictRegistry.remove(oreDictionaryKey);
    }

    @Override
    public void removeCompostMaterial(Class clazz) {
        if (clazz != null) classRegistry.remove(clazz);
    }
    ...
    public ICompostMaterial getCompostMaterialInfo(ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ItemBlock) {
            ...
            while (clazz != null) {
                if (classRegistry.containsKey(clazz)) return classRegistry.get(clazz);

                for (Class iface : clazz.getInterfaces()) {
                    if (classRegistry.containsKey(iface)) return classRegistry.get(iface);
                }

                clazz = clazz.getSuperclass();
            }
        } else if (itemStack.getItem() != null) {
            Class clazz = itemStack.getItem()
                .getClass();
