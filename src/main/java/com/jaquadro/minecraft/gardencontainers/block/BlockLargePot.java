...
public abstract class BlockLargePot extends BlockGardenContainer implements IChainAttachable, ISoilContainer {
    ...
    public BlockLargePot(String blockName) {
        ...
        slotProfile = new LocalSlotProfile(
            this,
            new LocalSlotProfile.Slot[] { new BasicSlotProfile.Slot(Slot14Profile.SLOT_CENTER, commonType, allSize),
    ...
    public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB mask, List list,
        ...
        else setBlockBounds(0, 0, 0, 1, 1 - dim, 1);
    ...
    public boolean shouldSideBeRendered(IBlockAccess blockAccess, int x, int y, int z, int side) {
        ...
        if (side >= 2 && side < 6) {
            ...
            if (te != null) return !te.isAttachedNeighbor(nx, y, nz);
        }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (item != null) items.add(new ItemStack(item, 1, packedMeta));
        }
    ...
    public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction,
        ...
        if (plantType == EnumPlantType.Crop) return substrateSupportsCrops(gardenTile.getSubstrate());

        return false;
    }

    protected boolean substrateSupportsCrops(ItemStack substrate) {
        if (substrate == null || substrate.getItem() == null) return false;

        if (Block.getBlockFromItem(substrate.getItem()) == ModBlocks.gardenFarmland) return true;
        if (Block.getBlockFromItem(substrate.getItem()) == Blocks.farmland) return true;
    ...
    protected boolean applySubstrateToGarden(World world, int x, int y, int z, EntityPlayer player, int slot,
        ItemStack itemStack) {
        if (getGardenSubstrate(world, x, y, z, slot) != null) return false;
    ...
    protected boolean applyHoeToSubstrate(World world, int x, int y, int z, TileEntityGarden tile,
        ...
        Block substrate = Block.getBlockFromItem(
            tile.getSubstrate()
                .getItem());
        if (substrate == Blocks.dirt || substrate == Blocks.grass) tile.setSubstrate(
            new ItemStack(Blocks.farmland, 1, 7),
            new ItemStack(
                Blocks.dirt,
                1,
        ...
        world.playSoundEffect(
            ...
            (Blocks.farmland.stepSound.getVolume() + 1) / 2f,
            Blocks.farmland.stepSound.getPitch() * .8f);
    ...
    public boolean applyHoe(World world, int x, int y, int z) {
        ...
        if (te != null && te.isEmpty()) return applyHoeToSubstrate(world, x, y, z, te, null);
    ...
    private static final Vec3[] chainAttachPoints = new Vec3[] { Vec3.createVectorHelper(.03125 + .005, 1, .03125),
        Vec3.createVectorHelper(.03125, 1, 1 - .03125 - .005), Vec3.createVectorHelper(1 - .03125, 1, .03125 + .005),
        Vec3.createVectorHelper(1 - .03125 - .005, 1, 1 - .03125), };

    @Override
    public Vec3[] getChainAttachPoints(int side) {
        if (side == 1) return chainAttachPoints;
    ...
    public IIcon getOverlayIcon(int data) {
        if (iconOverlayArray[data] != null) return iconOverlayArray[data];
