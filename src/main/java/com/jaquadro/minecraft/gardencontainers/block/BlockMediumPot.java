...
public abstract class BlockMediumPot extends BlockGardenContainer implements IChainAttachable {
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (item != null) items.add(new ItemStack(item, 1, metadata));
        }
    ...
    private static final Vec3[] chainAttachPoints = new Vec3[] {
        Vec3.createVectorHelper(.125f + .03125 + .005, .75f, .125f + .03125),
        Vec3.createVectorHelper(.125f + .03125, .75f, 1 - .125f - .03125 - .005),
        Vec3.createVectorHelper(1 - .125f - .03125, .75f, .125f + .03125 + .005),
        Vec3.createVectorHelper(1 - .125f - .03125 - .005, .75f, 1 - .125f - .03125), };
