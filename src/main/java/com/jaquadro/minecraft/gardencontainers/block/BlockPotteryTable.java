...
public class BlockPotteryTable extends BlockContainer {
    ...
    private void setBlockDirection(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            byte dir = 3;
            if (blockZNeg.func_149730_j() && !blockZPos.func_149730_j()) dir = 3;
            if (blockZPos.func_149730_j() && !blockZNeg.func_149730_j()) dir = 2;
            if (blockXNeg.func_149730_j() && !blockXPos.func_149730_j()) dir = 5;
            if (blockXPos.func_149730_j() && !blockXNeg.func_149730_j()) dir = 4;
    ...
    public IIcon getIcon(int side, int data) {
        if (side == 0) return Blocks.planks.getBlockTextureFromSide(side);
        if (side == 1) return iconTop;
        if (side == data) return blockIcon;
        return iconSide;
    }
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        blockIcon = iconRegister.registerIcon(GardenContainers.MOD_ID + ":pottery_table_front");
        iconSide = iconRegister.registerIcon(GardenContainers.MOD_ID + ":pottery_table_side");
        iconTop = iconRegister.registerIcon(GardenContainers.MOD_ID + ":pottery_table_top");
    }
}
