...
public class BlockWindowBox extends BlockGarden {
    ...
    private class LocalSlotProfile extends Slot5Profile {
        ...
        public float getPlantOffsetY(IBlockAccess blockAccess, int x, int y, int z, int slot) {
            ...
            if (garden == null || garden.isUpper()) return -.0625f;
            else return -.5f - .0625f;
        }
    ...
    protected int getSlot(World world, int x, int y, int z, EntityPlayer player, float hitX, float hitY, float hitZ) {
        ...
        if (hitX <= .5) {
            if (hitZ <= .5 && tileEntity.isSlotValid(Slot5Profile.SLOT_NW)) return Slot5Profile.SLOT_NW;
            else if (tileEntity.isSlotValid(Slot5Profile.SLOT_SW)) return Slot5Profile.SLOT_SW;
        } else {
            if (hitZ <= .5 && tileEntity.isSlotValid(Slot5Profile.SLOT_NE)) return Slot5Profile.SLOT_NE;
            else if (tileEntity.isSlotValid(Slot5Profile.SLOT_SE)) return Slot5Profile.SLOT_SE;
        }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack itemStack) {
        ...
        if (te == null || te.getDirection() != 0) return;
