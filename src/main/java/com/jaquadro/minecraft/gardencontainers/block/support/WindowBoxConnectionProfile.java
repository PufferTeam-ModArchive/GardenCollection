...
public class WindowBoxConnectionProfile extends BasicConnectionProfile {
    ...
    public boolean isAttachedNeighbor(IBlockAccess blockAccess, int x, int y, int z, int nx, int ny, int nz) {
        if (!super.isAttachedNeighbor(blockAccess, x, y, z, nx, ny, nz)) return false;
        ...
        if (ste == null || nte == null) return false;

        if (ste.isUpper() != nte.isUpper()) return false;
        ...
        if (dir == xzDir || rdir == xzDir) {
            ...
            TileEntity te = null;
            if (dir == 2) te = blockAccess.getTileEntity(nx, ny, nz - 1);
            else if (dir == 3) te = blockAccess.getTileEntity(nx, ny, nz + 1);
            else if (dir == 4) te = blockAccess.getTileEntity(nx - 1, ny, nz);
            else if (dir == 5) te = blockAccess.getTileEntity(nx + 1, ny, nz);
        ...
        switch (dir) {
            case 2:
            case 3:
                if (nx - x != 0 && ndir == dir) break;
                if (nx - x < 0 && ndir == 4) break;
                if (nx - x > 0 && ndir == 5) break;
                return false;
            case 4:
            case 5:
                if (nz - z != 0 && ndir == dir) break;
                if (nz - z < 0 && ndir == 2) break;
                if (nz - z > 0 && ndir == 3) break;
                return false;
            default:
                return false;
        }
