...
public class WindowBoxRenderer implements ISimpleBlockRenderingHandler {
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
        if (!(block instanceof BlockWindowBox)) return;
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (!(block instanceof BlockWindowBox)) return false;
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockWindowBox block, int modelId,
        ...
        if (validNW) {
            int connect = 0 | (validNE ? ModularBoxRenderer.CONNECT_XPOS : 0)
                | (validSW ? ModularBoxRenderer.CONNECT_ZPOS : 0);
            boxRenderer
                .renderOctant(world, block, x, y + (te.isUpper() ? .5 : 0), z, connect, ModularBoxRenderer.CUT_YPOS);
        }
        if (validNE) {
            int connect = 0 | (validNW ? ModularBoxRenderer.CONNECT_XNEG : 0)
                | (validSE ? ModularBoxRenderer.CONNECT_ZPOS : 0);
            boxRenderer.renderOctant(
                world,
                block,
                x + .5,
                y + (te.isUpper() ? .5 : 0),
        ...
        if (validSW) {
            int connect = 0 | (validSE ? ModularBoxRenderer.CONNECT_XPOS : 0)
                | (validNW ? ModularBoxRenderer.CONNECT_ZNEG : 0);
            boxRenderer.renderOctant(
                world,
                block,
                x,
        ...
        if (validSE) {
            int connect = 0 | (validSW ? ModularBoxRenderer.CONNECT_XNEG : 0)
                | (validNE ? ModularBoxRenderer.CONNECT_ZNEG : 0);
            boxRenderer.renderOctant(
                world,
                block,
                x + .5,
