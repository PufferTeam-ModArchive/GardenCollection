...
public class ItemLargePot extends ItemMultiTexture {
    ...
    private static String[] getSubTypes(Block block) {
        if (block instanceof BlockLargePot) return ((BlockLargePot) block).getSubTypes();
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        if (i < 0 || i >= this.field_150942_c.length) i = 0;
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean par4) {
        ...
        if (pattern != null && pattern.getName() != null) list.add(pattern.getName());
    }
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (largePot != null) largePot.setCarving((metadata >> 8) & 255);
