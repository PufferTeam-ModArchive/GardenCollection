...
public abstract class BlockGardenContainer extends BlockGarden {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int data) {
        ...
        if (te != null) {
            ...
            if (substrate == null) substrate = te.getSubstrate();
            if (substrate != null) {
    ...
    protected boolean applyItemToGarden(World world, int x, int y, int z, EntityPlayer player, ItemStack itemStack,
        ...
        if (applySubstrateToGarden(world, x, y, z, (itemStack == null) ? player : null, slot, item)) return true;
    ...
    protected boolean applySubstrateToGarden(World world, int x, int y, int z, EntityPlayer player, int slot,
        ...
        if (!isValidSubstrate(world, x, y, z, slot, itemStack)) return false;

        TileEntityGarden garden = getTileEntity(world, x, y, z);

        ItemStack translation = translateSubstrate(world, x, y, z, slot, itemStack);
        if (translation == null || translation == itemStack) garden.setSubstrate(itemStack);
        else garden.setSubstrate(itemStack, translation);
    ...
    protected boolean isPlantValidForSubstrate(ItemStack substrate, PlantItem plant) {
        ...
        switch (plant.getPlantTypeClass()) {
            ...
            case AQUATIC_SURFACE:
                if (Block.getBlockFromItem(substrate.getItem()) != Blocks.water) return false;
                break;
            case GROUND:
            case GROUND_COVER:
                if (Block.getBlockFromItem(substrate.getItem()) == Blocks.water) return false;
    ...
    protected ItemStack translateSubstrate(World world, int x, int y, int z, int slot, ItemStack itemStack) {
        if (Block.getBlockFromItem(itemStack.getItem()) == Blocks.farmland) return new ItemStack(Blocks.farmland, 1, 1);
