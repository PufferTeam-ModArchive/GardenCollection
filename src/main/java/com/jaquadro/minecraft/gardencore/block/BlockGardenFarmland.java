...
public class BlockGardenFarmland extends BlockGarden {
    ...
    public BlockGardenFarmland(String blockName) {
        ...
        if (GardenCore.config.enableTilledSoilGrowthBonus) setTickRandomly(true);
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        if (block instanceof IPlantable || block instanceof IGrowable) block.updateTick(world, x, y + 1, z, random);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        if (side == 1) return iconTilledSoil;
        return blockIcon;
    }
