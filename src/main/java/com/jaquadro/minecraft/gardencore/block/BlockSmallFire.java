...
public class BlockSmallFire extends Block {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        if (!blockCanHostSmallFlame(world, x, y - 1, z)) world.setBlockToAir(x, y, z);
    }

    @Override
    public void onBlockAdded(World world, int x, int y, int z) {
        if (!blockCanHostSmallFlame(world, x, y - 1, z)) world.setBlockToAir(x, y, z);
        else world.scheduleBlockUpdate(x, y, z, this, tickRate(world) + world.rand.nextInt(10));
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        if (random.nextInt(64) == 0) world.playSound(
            x + .5f,
            y + .5f,
            z + .5f,
            "fire.fire",
    ...
    public void registerBlockIcons(IIconRegister register) {
        icons = new IIcon[] { register.registerIcon(GardenCore.MOD_ID + ":" + getTextureName() + "_layer_0"),
