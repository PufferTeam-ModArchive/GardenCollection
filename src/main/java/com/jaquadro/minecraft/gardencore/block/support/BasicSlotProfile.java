...
public class BasicSlotProfile implements ISlotProfile {
    ...
    public static class Slot {
        ...
        public Slot(int slot, PlantType[] typeClasses, PlantSize[] sizeClasses) {
            ...
            for (int i = 0; i < typeClasses.length; i++) validTypeClasses.add(typeClasses[i]);
            for (int i = 0; i < sizeClasses.length; i++) validSizeClasses.add(sizeClasses[i]);
        }
    }
    ...
    public BasicSlotProfile(Slot[] slots) {
        ...
        for (int i = 0; i < slots.length; i++) slotIndexes[i] = slots[i].slot;
    }
    ...
    public boolean isPlantValidForSlot(IBlockAccess blockAccess, int x, int y, int z, int slot, PlantItem plant) {
        ...
        if (!slots[slot].validTypeClasses.contains(plant.getPlantTypeClass())) return false;
        if (!slots[slot].validSizeClasses.contains(plant.getPlantSizeClass())) return false;
