...
public class Slot2Profile extends BasicSlotProfile {
    ...
    private static float[] plantOffsetX = new float[] { 0, 0 };

    private static float[] plantOffsetZ = new float[] { 0, 0 };

    public Slot2Profile(BasicSlotProfile.Slot[] slots) {
        super(slots);

        if (slots.length != 2) throw new IllegalArgumentException("Invalid slot count");
    }
