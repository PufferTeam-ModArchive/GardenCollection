...
public class Slot5Profile extends BasicSlotProfile {
    ...
    private static float[] plantOffsetX = new float[] { 0, -.252f, .25f, -.25f, .252f };

    private static float[] plantOffsetZ = new float[] { 0, -.25f, -.252f, .252f, .25f };

    public Slot5Profile(Slot[] slots) {
        super(slots);

        if (slots.length != 5) throw new IllegalArgumentException("Invalid slot count");
    }
