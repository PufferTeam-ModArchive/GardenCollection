...
public class TileEntityGarden extends TileEntity implements IInventory {
    ...
    public void clearPlantedContents() {
        ...
        if (block != null) clearReachableContents(0, containerSlotCount());
    }
    ...
    public boolean isEmpty() {
        for (int i = 0; i < containerStacks.length; i++) {
            if (getPlantInSlot(i) != null) return false;
        }
    ...
    private void clearReachableContents(int start, int length) {
        for (int i = start; i < length; i++) setInventorySlotContents(i, null, false);
    ...
    public boolean isSlotValid(int slot) {
        if (!isSharedSlot(slot)) return true;

        BlockGarden block = getGardenBlock();
        for (ISlotMapping mapping : block.getSlotShareProfile()
            .getNeighborsForSlot(slot)) {
            if (!block.getConnectionProfile()
                .isAttachedNeighbor(
                    ...
                    yCoord,
                    zCoord,
                    xCoord + mapping.getMappedX(),
                    yCoord,
                    zCoord + mapping.getMappedZ()))
                return false;
        }
    ...
    public boolean isAttachedNeighbor(int nx, int ny, int nz) {
        ...
        return garden.getConnectionProfile()
            .isAttachedNeighbor(worldObj, xCoord, yCoord, zCoord, nx, ny, nz);
    }

    public BlockGarden getGardenBlock() {
        Block block = worldObj.getBlock(xCoord, yCoord, zCoord);
        if (block instanceof BlockGarden) return (BlockGarden) block;
    ...
    private ItemStack getStackLocal(int slot) {
        if (containerPlants == null && hasWorldObj()) initializeContainerPlants();

        if (containerPlants != null) return containerPlants[slot].getPlantSourceItem();
        else return containerStacks[slot];
    }

    private void setStackLocal(int slot, ItemStack itemStack) {
        if (containerPlants == null && hasWorldObj()) initializeContainerPlants();

        if (containerPlants != null && hasWorldObj()) containerPlants[slot] = PlantItem.getForItem(worldObj, itemStack);
    ...
    private void initializeContainerPlants() {
        ...
        for (int i = 0; i < containerStacks.length; i++) {
            if (containerStacks[i] != null) containerPlants[i] = PlantItem.getForItem(worldObj, containerStacks[i]);
        }
    }
    ...
    public void setWorldObj(World world) {
        ...
        if (containerPlants == null && world != null) initializeContainerPlants();
        else if (world == null) containerPlants = null;
    }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < itemList.tagCount(); i++) {
            ...
            if (!item.hasKey("Slot") || !item.hasKey("Item") || !item.hasKey("Data")) continue;

            byte slot = item.getByte("Slot");
            if (slot < 0 || slot >= containerStacks.length) continue;
        ...
        if (tag.hasKey("SubId")) {
            ...
            if (tag.hasKey("SubDa")) substrate.setItemDamage(tag.getShort("SubDa"));

            if (tag.hasKey("SubSrcId")) {
                substrateSource = new ItemStack(Item.getItemById(tag.getShort("SubSrcId")));
                if (tag.hasKey("SubSrcDa")) substrateSource.setItemDamage(tag.getShort("SubSrcDa"));
                if (tag.hasKey("SubSrcTag", 10)) substrateSource.stackTagCompound = tag.getCompoundTag("SubSrcTag");
            }
        }
    ...
    public void writeToNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < containerStacks.length; i++) {
            if (containerStacks[i] == null || containerStacks[i].getItem() == null) continue;
        ...
        if (substrate != null) {
            ...
            if (substrate.getItemDamage() != 0) tag.setShort("SubDa", (short) substrate.getItemDamage());
            ...
            if (substrateSource != null) {
                ...
                if (substrateSource.stackTagCompound != null) tag.setTag("SubSrcTag", substrateSource.stackTagCompound);
            }
        }

        if (hasBiomeOverride || biomeData != DEFAULT_BIOME_DATA) tag.setInteger("Biom", biomeData);
    }
    ...
    public ItemStack getStackInSlot(int slot) {
        if (!isSharedSlot(slot)) return getStackInSlotIsolated(slot);

        if (!isSlotValid(slot)) return null;
        ...
        for (ISlotMapping mapping : block.getSlotShareProfile()
            ...
            TileEntity te = worldObj
                .getTileEntity(xCoord + mapping.getMappedX(), yCoord, zCoord + mapping.getMappedZ());
            if (te == null || !(te instanceof TileEntityGarden)) continue;
    ...
    protected void setInventorySlotContentsIsolated(int slot, ItemStack itemStack, boolean notify) {
        ...
        if (notify) BlockGarden.validateBlockState(this);
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack itemStack) {
        ...
        if (!block.isPlantValidForSlot(worldObj, xCoord, yCoord, zCoord, slot, plant)) return false;
