...
public class PlantMegaPackIntegration {
    ...
    public static void init() {
        ...
        GardenCoreAPI.instance()
            .registerBonemealHandler(new BonemealHandler());
    ...
    public static class BonemealHandler implements IBonemealHandler {
        ...
        public boolean applyBonemeal(World world, int x, int y, int z, BlockGarden hostBlock, int slot) {
            ...
            if (block == null || !(block instanceof PMPBlockPlant)) return false;

            PMPBlockPlant plantBlock = (PMPBlockPlant) block;
            if (plantBlock.isFullyGrown(meta)) return false;
        ...
        private int growMeta(PMPBlockPlant block, int meta) {
            if (meta == 0) return 1;
            if (meta == 1 && block.plantData.attributes.growthStages > 2) return 2;
            if (meta == 2 && block.plantData.attributes.growthStages > 3) return 4;
            if (meta == 4 && block.plantData.attributes.growthStages > 4) return 6;
    ...
    public static class PlantInfo implements IPlantInfo {
        ...
        public int getPlantHeight(Block block, int meta) {
            if (block == null || !(block instanceof PMPBlockPlant)) return 1;

            PMPBlockPlant plantBlock = (PMPBlockPlant) block;
            if (plantBlock.plantData.attributes.growthType == PMPPlantGrowthType.DOUBLE && meta >= 2) return 2;
        ...
        public int getPlantSectionMeta(Block block, int meta, int section) {
            if (block == null || !(block instanceof PMPBlockPlant)) return meta;
        ...
        public PlantType getPlantTypeClass(Block block, int meta) {
            if (block == null || !(block instanceof PMPBlockPlant)) return PlantType.INVALID;
        ...
        public int getPlantMaxHeight(Block block, int meta) {
            ...
            PMPBlockPlant plantBlock = (PMPBlockPlant) block;
            if (plantBlock.plantData.attributes.growthType == PMPPlantGrowthType.DOUBLE) return 2;
