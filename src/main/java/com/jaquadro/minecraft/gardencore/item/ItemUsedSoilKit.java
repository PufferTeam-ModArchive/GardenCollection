...
public class ItemUsedSoilKit extends Item {
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean par4) {
        ...
        if (temperature >= .2) tempColor = EnumChatFormatting.DARK_GREEN;
        if (temperature >= 1) tempColor = EnumChatFormatting.DARK_RED;

        EnumChatFormatting humidColor = EnumChatFormatting.DARK_RED;
        if (humidity >= .3) humidColor = EnumChatFormatting.GOLD;
        if (humidity >= .6) humidColor = EnumChatFormatting.DARK_GREEN;
