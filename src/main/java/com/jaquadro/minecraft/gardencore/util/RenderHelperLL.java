...
public class RenderHelperLL {
    ...
    private void drawFaceY(int face, double x, double y, double z, IIcon icon) {
        ...
        if (rangeX <= 1 && rangeZ <= 1) {
            setUV(
                icon,
                state.renderMinX + state.shiftU,
                state.renderMinZ + state.shiftV,
                state.renderMaxX + state.shiftU,
                state.renderMaxZ + state.shiftV);

            if (state.enableAO) renderXYZUVAO(xyzuvMap[face]);
            else renderXYZUV(xyzuvMap[face]);
            return;
        }
    ...
    private void drawFaceZ(int face, double x, double y, double z, IIcon icon) {
        ...
        if (rangeX <= 1 && rangeY <= 1) {
            if (state.flipTexture) setUV(
                icon,
                state.renderMaxX + state.shiftU,
                1 - state.renderMaxY + state.shiftV,
                state.renderMinX + state.shiftU,
                1 - state.renderMinY + state.shiftV);
            else setUV(
                icon,
                state.renderMinX + state.shiftU,
                1 - state.renderMaxY + state.shiftV,
        ...
        for (int ix = 0; ix < rangeX; ix++) {
            ...
            for (int iy = 0; iy < rangeY; iy++) {
                ...
                if (state.flipTexture) setUV(icon, 1 - minUDiv[ix], 1 - maxVDiv[iy], 1 - maxUDiv[ix], 1 - minVDiv[iy]);
                else setUV(icon, minUDiv[ix], 1 - maxVDiv[iy], maxUDiv[ix], 1 - minVDiv[iy]);
    ...
    private void drawFaceX(int face, double x, double y, double z, IIcon icon) {
        ...
        if (rangeZ <= 1 && rangeY <= 1) {
            if (state.flipTexture) setUV(
                icon,
                state.renderMaxZ + state.shiftU,
                1 - state.renderMaxY + state.shiftV,
                state.renderMinZ + state.shiftU,
                1 - state.renderMinY + state.shiftV);
            else setUV(
                icon,
                state.renderMinZ + state.shiftU,
        ...
        for (int iz = 0; iz < rangeZ; iz++) {
            ...
            for (int iy = 0; iy < rangeY; iy++) {
                ...
                if (state.flipTexture) setUV(icon, 1 - minUDiv[iz], 1 - maxVDiv[iy], 1 - maxUDiv[iz], 1 - minVDiv[iy]);
                else setUV(icon, minUDiv[iz], 1 - maxVDiv[iy], maxUDiv[iz], 1 - minVDiv[iy]);
