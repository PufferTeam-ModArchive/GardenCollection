...
public abstract class BlockConnected extends Block {
    ...
    public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB mask, List list,
        ...
        if (flagW || flagE) {
            if (flagW && flagE) this.setBlockBounds(0.0F, 0.0F, 0.4375F, 1.0F, height, 0.5625F);
            else if (flagW) this.setBlockBounds(0.0F, 0.0F, 0.4375F, 0.5F, height, 0.5625F);
            else if (flagE) this.setBlockBounds(0.5F, 0.0F, 0.4375F, 1.0F, height, 0.5625F);

            super.addCollisionBoxesToList(world, x, y, z, mask, list, entity);
        }

        if (flagN || flagS) {
            if (flagN && flagS) this.setBlockBounds(0.4375F, 0.0F, 0.0F, 0.5625F, height, 1.0F);
            else if (flagN) this.setBlockBounds(0.4375F, 0.0F, 0.0F, 0.5625F, height, 0.5F);
            else if (flagS) this.setBlockBounds(0.4375F, 0.0F, 0.5F, 0.5625F, height, 1.0F);
    ...
    public boolean canConnectTo(IBlockAccess world, int x, int y, int z, ForgeDirection dir) {
        return canConnectTo(world.getBlock(x, y, z)) || world.isSideSolid(x, y, z, dir.getOpposite(), false);
    }
