...
public class BlockLantern extends BlockContainer implements IInfusionStabiliser {
    ...
    public boolean isGlass(ItemStack item) {
        if (item.hasTagCompound()) {
            ...
            if (tag.hasKey("glass")) return tag.getBoolean("glass");
        }
    ...
    public String getLightSource(ItemStack item) {
        if (item.hasTagCompound()) {
            NBTTagCompound tag = item.getTagCompound();
            if (tag.hasKey("src", Constants.NBT.TAG_STRING)) return tag.getString("src");
        }
    ...
    public int getLightSourceMeta(ItemStack item) {
        if (item.hasTagCompound()) {
            NBTTagCompound tag = item.getTagCompound();
            if (tag.hasKey("srcMeta")) return tag.getShort("srcMeta");
        }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (lanternSource != null) lanternSource.renderParticle(world, x, y, z, rand, tile.getLightSourceMeta());
    }
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        ...
        ILanternSource lanternSource = Api.instance.registries.lanternSources.getLanternSource(tile.getLightSource());
        if (lanternSource != null) return lanternSource.getLightLevel(tile.getLightSourceMeta());
    ...
    public int isProvidingStrongPower(IBlockAccess world, int x, int y, int z, int side) {
        ...
        return (side == 1 && source != null)
            ? source.strongPowerValue(getTileEntity(world, x, y, z).getLightSourceMeta())
            : 0;
    }
    ...
    private IRedstoneSource getRedstoneSource(IBlockAccess world, int x, int y, int z) {
        ...
        if (tile != null) {
            ...
            if (source instanceof IRedstoneSource) return (IRedstoneSource) source;
        }
    ...
    public boolean canStabaliseInfusion(World world, int x, int y, int z) {
        ...
        ILanternSource lanternSource = Api.instance.registries()
            .lanternSources()
            .getLanternSource(tile.getLightSource());
        if (lanternSource == null) return false;

        return lanternSource.getSourceID()
