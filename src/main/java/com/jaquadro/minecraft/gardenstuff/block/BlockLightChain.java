...
public class BlockLightChain extends Block implements IPlantProxy, IChain {
    ...
    private static final Vec3[] defaultAttachPoints = new Vec3[] { Vec3.createVectorHelper(.03125, 1, .03125),
        Vec3.createVectorHelper(.03125, 1, 1 - .03125), Vec3.createVectorHelper(1 - .03125, 1, .03125),
        Vec3.createVectorHelper(1 - .03125, 1, 1 - .03125), };
    private static final Vec3[] singleAttachPoint = new Vec3[] { Vec3.createVectorHelper(.5, 1, .5), };
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        for (Vec3 point : getAttachPoints(world, x, y, z)) {
            if (point.xCoord < minX) minX = (float) point.xCoord;
            if (point.zCoord < minZ) minZ = (float) point.zCoord;
            if (point.xCoord > maxX) maxX = (float) point.xCoord;
            if (point.zCoord > maxZ) maxZ = (float) point.zCoord;
        }
    ...
    public int findMinY(IBlockAccess world, int x, int y, int z) {
        while (y > 0) {
            if (world.getBlock(x, --y, z) != this) return y + 1;
        }
    ...
    public Vec3[] getAttachPoints(IBlockAccess world, int x, int y, int z) {
        ...
        if (bottomBlock instanceof IChainAttachable)
        ...
        else if (attachable != null && attachable.isAttachable(world, x, y - 1, z, 1)) attachPoints = new Vec3[] {
            Vec3.createVectorHelper(.5, attachable.getAttachDepth(world, x, y - 1, z, 1), .5) };
            ...
            if (attachPoint != null) attachPoints = new Vec3[] { attachPoint };
    ...
    public void registerBlockIcons(IIconRegister register) {
        ...
        for (int i = 0; i < types.length; i++) icons[i] = register.registerIcon(getTextureName() + "_" + types[i]);
    }
