...
public class TileEntityBloomeryFurnace extends TileEntity implements ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            if (furnaceBurnTime != 0 || (furnaceItemStacks[SLOT_FUEL] != null && furnaceItemStacks[SLOT_PRIMARY] != null
                    ...
                    if (furnaceBurnTime > 0) {
                        ...
                        if (furnaceItemStacks[SLOT_FUEL] != null) {
                            ...
                            if (furnaceItemStacks[SLOT_FUEL].stackSize == 0)
                                furnaceItemStacks[SLOT_FUEL] = furnaceItemStacks[SLOT_FUEL].getItem()
                                    .getContainerItem(furnaceItemStacks[SLOT_FUEL]);
                        }
                    }
                }
    ...
    private boolean canSmelt() {
        if (furnaceItemStacks[SLOT_PRIMARY] == null || furnaceItemStacks[SLOT_SECONDARY] == null) return false;

        if (!isItemPrimaryInput(furnaceItemStacks[SLOT_PRIMARY])) return false;
        if (!isItemSecondaryInput(furnaceItemStacks[SLOT_SECONDARY])) return false;

        ItemStack itemOutput = new ItemStack(ModItems.wroughtIronIngot);
        if (furnaceItemStacks[SLOT_OUTPUT] == null) return true;
        if (!furnaceItemStacks[SLOT_OUTPUT].isItemEqual(itemOutput)) return false;
    ...
    public void smeltItem() {
        ...
        if (furnaceItemStacks[SLOT_OUTPUT] == null) furnaceItemStacks[SLOT_OUTPUT] = itemOutput.copy();
        ...
        if (furnaceItemStacks[SLOT_PRIMARY].stackSize <= 0) furnaceItemStacks[SLOT_PRIMARY] = null;
        if (furnaceItemStacks[SLOT_SECONDARY].stackSize <= 0) furnaceItemStacks[SLOT_SECONDARY] = null;
    }
    ...
    public static int getItemBurnTime(ItemStack stack) {
        ...
        if (item == Items.coal && stack.getItemDamage() == 1) return 1600;
    ...
    public static boolean isItemPrimaryInput(ItemStack stack) {
        ...
        if (stack.getItem() == Item.getItemFromBlock(Blocks.iron_ore)) return true;
    ...
    public static boolean isItemSecondaryInput(ItemStack stack) {
        ...
        if (stack.getItem() == Item.getItemFromBlock(Blocks.sand)) return true;
    ...
    public ItemStack decrStackSize(int slot, int count) {
        if (furnaceItemStacks[slot] != null) {
            if (furnaceItemStacks[slot].stackSize <= count) {
                ...
                if (furnaceItemStacks[slot].stackSize == 0) furnaceItemStacks[slot] = null;
                return stack;
            }
        }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (slot == SLOT_FUEL) return isItemFuel(stack);
        if (slot == SLOT_PRIMARY) return isItemPrimaryInput(stack);
        if (slot == SLOT_SECONDARY) return isItemSecondaryInput(stack);
