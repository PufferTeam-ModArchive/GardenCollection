...
public class TileEntityLantern extends TileEntity {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("Glas")) hasGlass = tag.getBoolean("Glas");
        ...
        if (tag.hasKey("Src", Constants.NBT.TAG_BYTE)) {
            ...
            if (index >= 0 && index < values.length) legacySource = values[index];
        ...
        } else if (tag.hasKey("Src", Constants.NBT.TAG_STRING)) source = tag.getString("Src");

        if (tag.hasKey("SrcMeta")) sourceMeta = tag.getShort("SrcMeta");
    }
    ...
    public void writeToNBT(NBTTagCompound tag) {
        ...
        if (hasGlass) tag.setBoolean("Glas", true);

        if (source != null) tag.setString("Src", source);

        if (sourceMeta != 0) tag.setShort("SrcMeta", (short) sourceMeta);
    }
