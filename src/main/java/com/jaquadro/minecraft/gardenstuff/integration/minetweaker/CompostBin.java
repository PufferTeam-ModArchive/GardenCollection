...
public class CompostBin {
    ...
    public static void add(IItemStack item, int processTime) {
        ...
        if (stack != null && stack.getItem() != null) MineTweakerAPI.apply(new AddItemAction(stack, processTime));
        else MineTweakerAPI.logError("Tried to add invalid item to compost table.");
    }
    ...
    public static void add(String oredictKey, int processTime) {
        ...
        else MineTweakerAPI.logError("Tried to add empty ore dictionary key to compost table.");
    }
    ...
    private static class AddItemAction implements IUndoableAction {
        ...
        public void apply() {
            GardenAPI.instance()
                .registries()
                .compost()
                .registerCompostMaterial(material, new StandardCompostMaterial(processTime, 0.125f));
        }
        ...
        public void undo() {
            GardenAPI.instance()
                .registries()
                .compost()
                .removeCompostMaterial(material);
        }

        @Override
        public String describe() {
            return "Adding item '" + material.getDisplayName()
    ...
    private static class AddOreAction implements IUndoableAction {
        ...
        @Override
        public String describe() {
            return "Adding ore dictionary key '" + material
                + "' to compost table with processing time '"
                + processTime
                + "'";
        }
