...
public class CandelabraRenderer implements ISimpleBlockRenderingHandler {
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
        if (!(block instanceof BlockCandelabra)) return;
    ...
    private void renderInventoryBlock(BlockCandelabra block, int metadata, int modelId, RenderBlocks renderer) {
        ...
        if (level >= 1) {
            ...
            RenderHelper.instance
                .renderFace(RenderHelper.ZNEG, renderer.blockAccess, block, block.getIconArmExt(), metadata);

            RenderHelper.instance.state.setRenderOffset(.5f, 0, 0);
            RenderHelper.instance.setRenderBounds(0, 0, 0, .5f, 1, .5f);
            RenderHelper.instance
                .renderFace(RenderHelper.ZPOS, renderer.blockAccess, block, block.getIconArmExt(), metadata);
        ...
        if (level >= 2) {
            ...
            RenderHelper.instance.setRenderBounds(.5f, 0, .5f, 1, 1, 1);
            RenderHelper.instance
                .renderFace(RenderHelper.XNEG, renderer.blockAccess, block, block.getIconArmExt(), metadata);

            RenderHelper.instance.state.setRenderOffset(0, 0, 0);
            RenderHelper.instance.setRenderBounds(0, 0, 0, .5f, 1, .5f);
            RenderHelper.instance
                .renderFace(RenderHelper.XPOS, renderer.blockAccess, block, block.getIconArmExt(), metadata);
    ...
    private void renderCandle(IBlockAccess world, BlockCandelabra block, int meta) {
        ...
        for (int i = 2; i < 6; i++) boxrender.setIcon(block.getIconCandleSide(), i);
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (!(block instanceof BlockCandelabra)) return false;
    ...
    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockCandelabra block, int modelId,
        ...
        if (tile.isSconce()) renderSconce(world, x, y, z, block, tile.getLevel());
        else renderCandelabra(world, x, y, z, block, tile.getLevel());
    ...
    private void renderCandelabra(IBlockAccess world, int x, int y, int z, BlockCandelabra block, int level) {
        ...
        if (hanging) RenderHelper.instance.drawCrossedSquares(block.getIconHang(), x, y, z, 1);
        else RenderHelper.instance.drawCrossedSquares(block.getIconBase(), x, y, z, 1);
