...
public class FenceRenderer implements ISimpleBlockRenderingHandler {
    ...
    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockFence block, int modelId,
        ...
        if (block.getPostInterval(meta) == 8) {
            if (connectZNeg) boxRenderer.renderSolidBox(world, block, x, y, z, U7, 0, 0, U9, 1, U1);
            if (connectZPos) boxRenderer.renderSolidBox(world, block, x, y, z, U7, 0, U15, U9, 1, 1);
            if (connectXNeg) boxRenderer.renderSolidBox(world, block, x, y, z, 0, 0, U7, U1, 1, U9);
            if (connectXPos) boxRenderer.renderSolidBox(world, block, x, y, z, U15, 0, U7, 1, 1, U9);
        }
        ...
        if (abNN) boxRenderer.setIcon(block.getIconTB(meta));

        float yN = 0;
        float yP = 1;

        if (!(abNN || abYY)) {
            if (connectYPos) yN = U8;
            else if (connectYNeg) yP = U8;
        }

        if (connectZNeg) boxRenderer.renderSolidBox(world, block, x, y, z, U8, yN, 0, U8, yP, U8);
        if (connectZPos) boxRenderer.renderSolidBox(world, block, x, y, z, U8, yN, U8, U8, yP, 1);
        if (connectXNeg) boxRenderer.renderSolidBox(world, block, x, y, z, 0, yN, U8, U8, yP, U8);
        if (connectXPos) boxRenderer.renderSolidBox(world, block, x, y, z, U8, yN, U8, 1, yP, U8);
