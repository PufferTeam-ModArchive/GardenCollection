...
public class HeavyChainRenderer implements ISimpleBlockRenderingHandler {
    ...
    private static final Vec3[] defaultAttachPoint = new Vec3[] { Vec3.createVectorHelper(.5, 0, .5),
        Vec3.createVectorHelper(.5, 1, .5), Vec3.createVectorHelper(.5, .5, 0), Vec3.createVectorHelper(.5, .5, 1),
        Vec3.createVectorHelper(0, .5, .5), Vec3.createVectorHelper(1, .5, .5), };
    ...
    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockHeavyChain block, int modelId,
        ...
        if (upperBlock instanceof IChainSingleAttachable) {
            ...
            if (attach != null && attach != defaultAttachPoint[0]) upperDepth = attach.yCoord;
        }
        ...
        if (lowerBlock instanceof IChainSingleAttachable) {
            ...
            if (attach != null && attach != defaultAttachPoint[1]) lowerDepth = attach.yCoord;
        }
