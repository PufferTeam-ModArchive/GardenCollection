...
public class BlockStrangePlant extends BlockFlower implements IShearable {
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (strangePlantDrops.length == 0) return ret;
        if (world.rand.nextDouble() > GardenTrees.config.strangePlantDropChance) return ret;
        ...
        for (int i = 0; i < count; i++) ret.add(item);
