...
public class TileEntityWoodProxy extends TileEntity {
    ...
    public static Block getBlockFromComposedMetadata(int metadata) {
        if (metadata >= 16) return GameData.blockRegistry.get(metadata & 0xFFF);
        else if (metadata < 4) return Blocks.log;
        else if (metadata < 8) return Blocks.log2;
        else return null;
    }

    public static int getMetaFromComposedMetadata(int metadata) {
        if (metadata >= 16) return (metadata >> 12) & 0xF;
    ...
    public static void syncTileEntityWithData(World world, int x, int y, int z, int metadata) {
        ...
        if (block != null) te.setProtoBlock(block, protoMeta);
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("P")) unpackUnifiedProtoData(tag.getInteger("P"));
    }
