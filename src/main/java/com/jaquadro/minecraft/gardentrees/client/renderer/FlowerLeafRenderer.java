...
public class FlowerLeafRenderer implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (!(block instanceof BlockFlowerLeaves)) return false;
    ...
    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockFlowerLeaves block, int modelId,
        ...
        if (icon != null) renderer.renderFaceYPos(block, x, y + offset, z, icon);

        icon = block.getFlowerIcon(world, x, y, z, meta, 2);
        if (icon != null) renderer.renderFaceZNeg(block, x, y, z - offset, icon);

        icon = block.getFlowerIcon(world, x, y, z, meta, 3);
        if (icon != null) renderer.renderFaceZPos(block, x, y, z + offset, icon);

        icon = block.getFlowerIcon(world, x, y, z, meta, 4);
        if (icon != null) renderer.renderFaceXNeg(block, x - offset, y, z, icon);

        icon = block.getFlowerIcon(world, x, y, z, meta, 5);
        if (icon != null) renderer.renderFaceXPos(block, x + offset, y, z, icon);
