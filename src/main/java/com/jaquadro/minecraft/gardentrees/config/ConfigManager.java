...
public class ConfigManager {
    ...
    public ConfigManager(File file) {
        ...
        Property propCompostGrowsOrnamentalTrees = config
            .get(Configuration.CATEGORY_GENERAL, "compostGrowsOrnamentalTrees", true);
    ...
    private void parseStrangePlantItems(Property property) {
        ...
        for (int i = 0; i < results.size(); i++) strangePlantDrops[i] = results.get(i);
    }

    public ItemStack[] getStrangePlantDrops() {
        if (strangePlantDrops == null) {
            Property propStrangePlantDrops = config
                .get(Configuration.CATEGORY_GENERAL, "strangePlantDrops", new String[0]);
