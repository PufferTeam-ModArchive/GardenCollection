...
public class WoodFenceRecipe implements IRecipe {
        ...
        for (int row = 0; row < size - 1; row++) {
            for (int col = 0; col < size - 2; col++) {
                ...
                if (string1 == null || string2 == null
                    ...
                    || !string1.isItemEqual(string2)) continue;

                ItemStack wood1 = inventory.getStackInRowAndColumn(col + 1, row);
                ItemStack wood2 = inventory.getStackInRowAndColumn(col + 1, row + 1);
                if (wood1 == null || wood2 == null || !wood1.isItemEqual(wood2)) continue;
                if (Block.getBlockFromItem(wood1.getItem()) != ModBlocks.thinLog) continue;
                ...
                if (!WoodRegistry.instance()
                    .contains(woodBlock, woodMeta)) continue;

                if (woodBlock == woodType.getBlock() && woodMeta == woodType.meta) return new ItemStack(
    ...
    public ItemStack getRecipeOutput() {
        return new ItemStack(
            ModBlocks.thinLogFence,
            2,
            TileEntityWoodProxy.composeMetadata(woodType.getBlock(), woodType.meta));
    }
