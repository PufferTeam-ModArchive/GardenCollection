...
public abstract class WorldGenOrnamentalTree extends WorldGenAbstractTree {
    ...
    protected static String transform(String pattern, LayerType type, int option) {
        ...
        for (int i = 0; i < groups; i++) {
            if (((option >> i) & 1) == 0) pattern = pattern.replace((char) ('0' + i), ' ');
            else pattern = pattern.replace((char) ('0' + i), 'X');
        }

        if (type == LayerType.LEAF) pattern = pattern.replace('T', 'X');
        else if (type == LayerType.TRUNK) pattern = pattern.replace('X', 'T');

        boolean flipH = ((option >>> 30) & 1) == 1;
        boolean flipV = ((option >>> 31) & 1) == 1;
        if (flipH && flipV) pattern = new StringBuilder(pattern).reverse()
            .toString();
        else if (flipH) {
            ...
            for (int y = 0; y < dim; y++) {
                int base = y * dim;
                for (int x = 0; x < dim; x++) sb.setCharAt(base + x, pattern.charAt(base + dim - x - 1));
            }
            pattern = sb.toString();
        } else if (flipV) {
            int dim = getPatternDim(pattern);
            ...
            for (int y = 0; y < dim; y++) {
                ...
                for (int x = 0; x < dim; x++) sb.setCharAt(base + x, pattern.charAt(base2 + x));
            }
            pattern = sb.toString();
        }
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (!canGenerate(world, x, y, z, height)) return false;
    ...
    private boolean canGenerate(World world, int x, int y, int z, int height) {
        ...
        if (y + height > 256) return false;

        return canGenerateTrunk(world, x, y, z, trunkHeight) && canGenerateCanopy(world, x, y, z, trunkHeight);
    }
    ...
    protected boolean canGenerateTrunk(World world, int x, int y, int z, int trunkHeight) {
        for (int iy = y; iy < y + trunkHeight; iy++) {
            if (!isReplaceable(world, x, iy, z)) return false;
        }
        return true;
    }
    ...
    protected void generateTrunk(World world, int x, int y, int z, int trunkHeight) {
        for (int iy = y; iy < y + trunkHeight + 1; iy++) generateBlock(world, x, iy, z, wood, metaWood);
    }
    ...
    protected void generatePattern(World world, int x, int y, int z, String pattern) {
        ...
        for (int ix = x - dim; ix <= x + dim; ix++) {
            for (int iz = z - dim; iz <= z + dim; iz++) {
                int index = (iz - z + dim) * (dim * 2 + 1) + (ix - x + dim);
                if (pattern.charAt(index) == 'X') generateBlock(world, ix, y, iz);
                else if (pattern.charAt(index) == 'T') generateTrunk(world, ix, y, iz);
            }
        }
    }
